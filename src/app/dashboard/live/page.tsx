...
const TransactionsIcon = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M17 9V7a5 5 0 00-10 0v2M5 12h14M12 15v2m0 4h.01"
    />
  </svg>
);
const StatisticsIcon = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M11 17a4 4 0 01-4-4V5a4 4 0 018 0v8a4 4 0 01-4 4zm0 0v2m0 0h2m-2 0H9"
    />
  </svg>
);
...
export default function LiveDashboardPage() {
  ...
  const [notificationSettings, setNotificationSettings] =
    useState<NotificationSettings>(notificationService.getSettings());
  ...
  const [transactionFilter, setTransactionFilter] = useState<
    "highest" | "recent" | "lowest" | "oldest" | "all"
  >("all");
  ...
  const [billStates, setBillStates] = useState(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (storedExpenses) {
        try {
          const expenses = JSON.parse(storedExpenses).filter(
            (exp) => exp.amount > 0,
          );
          const now = new Date();
  ...
  const handlePriorityChange = (idx, value) => {
    setBillStates((prev) =>
      prev.map((bill, i) => (i === idx ? { ...bill, priority: value } : bill)),
    );
  };
  const handleAutoPayChange = (idx) => {
    setBillStates((prev) =>
      prev.map((bill, i) =>
        i === idx ? { ...bill, autoPay: !bill.autoPay } : bill,
      ),
    );
  };
  ...
  const barChartLabels = useMemo(() => {
    ...
    return days.map(
      (day) => `${day} ${currentMonth.toLowerCase().slice(0, 3)}`,
    );
  }, [currentMonth]);
  ...
  const educationGoal = savingsGoals[0] || {
    name: "Education",
    target: 10000,
    current: 0,
  };
  const educationPercent = Math.round(
    (educationGoal.current / educationGoal.target) * 100,
  );
  ...
  const recentTransactions = useMemo(() => {
    return allTransactions
      .filter((tx) => tx.date >= prevMonthStart && tx.date < prevMonthEnd)
      .slice(0, 4);
  ...
  useEffect(() => {
    ...
    if (userBills.length === 0 && storedIncome && storedSplit) {
      ...
      const perCategory =
        Math.floor((needsAmount / categories.length) * 100) / 100;
      let remainder =
        Math.round((needsAmount - perCategory * categories.length) * 100) / 100;
      const now = new Date();
      const prevMonth = now.getMonth() === 0 ? 11 : now.getMonth() - 1;
      const prevMonthYear =
        now.getMonth() === 0 ? now.getFullYear() - 1 : now.getFullYear();
  ...
  const handleAddGoal = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newGoalName.trim() || !newGoalTarget || isNaN(Number(newGoalTarget)))
      return;
  ...
  const handleUpdateGoalCurrent = (index: number, value: number) => {
    setSavingsGoals((prev) =>
      prev.map((goal, i) => (i === index ? { ...goal, current: value } : goal)),
    );
  };
  ...
  const transactionsByMonth = filteredTransactions.reduce((acc, tx) => {
    const key = tx.date.toLocaleString("default", {
      month: "long",
      year: "numeric",
    });
    if (!acc[key]) acc[key] = [];
  ...
  const statsTotalIncome = statsCashFlowData.reduce(
    (sum, d) => sum + d.income,
    0,
  );
  const statsTotalExpenses = statsCashFlowData.reduce(
    (sum, d) => sum + d.expenses,
    0,
  );
  const statsTotalSavings = statsCashFlowData.reduce(
    (sum, d) => sum + d.savings,
    0,
  );
  const statsTotalInvestments = statsCashFlowData.reduce(
    (sum, d) => sum + d.investments,
    0,
  );
  const statsNetCashFlow =
    statsTotalIncome -
    statsTotalExpenses -
    statsTotalSavings -
  ...
  async function handleUploadReceipt(idx: any) {
    ...
    reader.onload = async (e) => {
      if (!e.target?.result) {
        setOcrResults((prev) => [
          ...prev,
          { name: file.name, text: "Could not read file." },
        ]);
        setOcrLoading(false);
        return;
      }
      try {
        ...
        setOcrResults((prev) => [
          ...prev,
          { name: file.name, text: "Could not extract text." },
        ]);
      }
      setOcrLoading(false);
    };
  ...
  const handlePlaidSuccess = async function (
    publicToken: string,
    metadata: any,
  ) {
    try {
  ...
  function calculateAnkTrustScore(transactions) {
    ...
    const past = transactions.filter(
      (tx) =>
        new Date(tx.date) < recent[recent.length - 1]?.date &&
        new Date(tx.date) > sixtyDaysAgo,
    );
    let behaviorPenalty = 0;
    recent.forEach((tx) => {
      const similar = past.find(
        (p) =>
          p.category === tx.category &&
          Math.abs(p.amount - tx.amount) < 0.2 * tx.amount,
      );
  ...
  function getAnkTrustCategory(score) {
    ...
    if (score >= 610)
      return {
        ...
        emoji: "🟡",
        label: "Moderate",
        message: "2FA triggered",
      };
    return {
      color: "bg-red-600",
      emoji: "🔴",
      label: "Risky",
      message: "Identity re-auth required, transfer blocked",
    };
  }
  ...
  if (typeof window !== "undefined") {
    try {
      expenseBreakdown = JSON.parse(
        localStorage.getItem("monthlyExpenses") || "[]",
      );
      monthlyExpenses = expenseBreakdown.reduce(
  ...
  // Place these at the top level of the component (inside LiveDashboardPage)
  const [savingsPurpose, setSavingsPurpose] = useState("");
  const [savingsAmount, setSavingsAmount] = useState("");
  const [savingsAutoSend, setSavingsAutoSend] = useState(false);
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-black to-gray-900 text-white pt-24 flex flex-row">
      ...
      <aside className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col py-8 px-4 min-h-screen sticky top-0 left-0 z-20">
        ...
        <nav className="flex flex-col gap-2">
          ...
          <button
            className={`flex items-center px-4 py-3 rounded-lg text-lg font-medium transition-colors ${activeSection === "accounts" ? "bg-blue-700 text-white" : "hover:bg-gray-800 text-gray-300"}`}
            onClick={() => setActiveSection("accounts")}
          >
            <AccountsIcon /> Accounts
          </button>
          <button
            className={`flex items-center px-4 py-3 rounded-lg text-lg font-medium transition-colors ${activeSection === "billpay" ? "bg-blue-700 text-white" : "hover:bg-gray-800 text-gray-300"}`}
            onClick={() => setActiveSection("billpay")}
          >
            <BillPayIcon /> Bill Pay
          </button>
          <button
            className={`flex items-center px-4 py-3 rounded-lg text-lg font-medium transition-colors ${activeSection === "settings" ? "bg-blue-700 text-white" : "hover:bg-gray-800 text-gray-300"}`}
            onClick={() => setActiveSection("settings")}
          >
            <SettingsIcon /> Settings
          </button>
          <button
            className={`flex items-center px-4 py-3 rounded-lg text-lg font-medium transition-colors ${activeSection === "plan" ? "bg-blue-700 text-white" : "hover:bg-gray-800 text-gray-300"}`}
            onClick={() => setActiveSection("plan")}
          >
            <PlanIcon /> Plan
          </button>
          <button
            className={`flex items-center px-4 py-3 rounded-lg text-lg font-medium transition-colors ${activeSection === "goals" ? "bg-blue-700 text-white" : "hover:bg-gray-800 text-gray-300"}`}
            onClick={() => setActiveSection("goals")}
          >
            <GoalsIcon /> Goals
          </button>
          <button
            className={`flex items-center px-4 py-3 rounded-lg text-lg font-medium transition-colors ${activeSection === "receipt" ? "bg-blue-700 text-white" : "hover:bg-gray-800 text-gray-300"}`}
            onClick={() => setActiveSection("receipt")}
          >
            <ReceiptIcon /> Receipt Capture
          </button>
          {/* Savings Bucket Option */}
          <button
            className={`flex items-center px-4 py-3 rounded-lg text-lg font-medium transition-colors ${activeSection === "savingsbucket" ? "bg-blue-700 text-white" : "hover:bg-gray-800 text-gray-300"}`}
            onClick={() => setActiveSection("savingsbucket")}
          >
            <span className="mr-2">💰</span> Savings Bucket
          </button>
        </nav>
      </aside>
      {/* Main Content */}
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="ml-6 space-y-2">
                          ...
                          {Object.entries(
                            notificationSettings.browser.types,
                          ).map(([type, enabled]) => (
                            <label
                              key={type}
                              className="flex items-center gap-2"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-yellow-300/90 text-gray-900 rounded-xl p-6 shadow flex flex-col justify-between min-h-[180px]">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="bg-yellow-400/80 rounded px-2 py-1 text-xs font-semibold">
                    ...
                    <div className="h-16 w-full bg-yellow-200 rounded mb-2 flex items-end">
                      {barChartValues.map((bar, i) => (
                        <div
                          key={i}
                          className={`w-1/6 mx-0.5 rounded`}
                          style={{
                            height: `${10 + (bar.income / monthlyIncome) * 50}px`,
                            background: "#fde047",
                          }}
                        ></div>
                      ))}
                  ...
                  <div className="bg-yellow-100 text-gray-900 rounded-xl p-6 shadow flex flex-col justify-between min-h-[180px]">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-3/4 h-2 bg-yellow-400 rounded-full relative">
                        <div
                          className="absolute left-0 top-0 h-2 bg-yellow-600 rounded-full"
                  ...
                  <div className="bg-gray-900 rounded-xl p-6 shadow flex flex-col justify-between min-h-[180px]">
                    ...
                    <div className="flex items-end gap-2 h-32 w-full mb-4">
                      {barChartValues.map((bar, i) => (
                        ...
                        >
                          <div
                            className="bg-teal-400 rounded-t-lg w-6"
                            style={{
                              height: `${40 + (bar.income / monthlyIncome) * 60}px`,
                            }}
                          ></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-yellow-200/80 text-gray-900 rounded-xl p-6 shadow flex flex-col items-center min-h-[200px]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="bg-green-200 text-green-800 rounded px-2 py-1 text-xs font-semibold">
                        $600.21
                      </span>
                    </div>
                    ...
                    <button className="mt-2 px-4 py-2 bg-gray-900 text-yellow-300 rounded-lg font-semibold shadow">
                      Add new card
                    </button>
                  </div>
                  {/* Top Spending */}
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="text-2xl font-bold text-gray-900">
                            {topSpending.length > 0
                              ? Math.round(
                                  (topSpending[0].amount / spentThisMonth) *
                                    100,
                                )
                              : 0}
                            %
              ...
              <div className="max-w-3xl mx-auto w-full">
                {(() => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className="text-lg md:text-xl font-semibold text-gray-100 text-center"
          ...
          <div className="max-w-3xl mx-auto py-12">
            ...
            <div className="flex gap-3 mb-6">
              <button
                ...
                className={`px-4 py-2 rounded-lg font-semibold ${transactionFilter === "all" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}
              >
                All
              </button>
              <button
                onClick={() => setTransactionFilter("highest")}
                className={`px-4 py-2 rounded-lg font-semibold ${transactionFilter === "highest" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}
              >
                Highest
              </button>
              <button
                onClick={() => setTransactionFilter("recent")}
                className={`px-4 py-2 rounded-lg font-semibold ${transactionFilter === "recent" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}
              >
                Most Recent
              </button>
              <button
                onClick={() => setTransactionFilter("lowest")}
                className={`px-4 py-2 rounded-lg font-semibold ${transactionFilter === "lowest" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}
              >
                Lowest
              </button>
              <button
                onClick={() => setTransactionFilter("oldest")}
                className={`px-4 py-2 rounded-lg font-semibold ${transactionFilter === "oldest" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}
              >
                Oldest
              </button>
            </div>
          ...
          <div className="max-w-4xl mx-auto py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
              <div className="bg-blue-900/80 rounded-xl p-6 text-white flex flex-col items-center shadow">
                ...
                <span className="text-2xl font-bold text-green-400">
                  {formatCurrency(statsTotalIncome)}
                </span>
              </div>
              <div className="bg-blue-900/80 rounded-xl p-6 text-white flex flex-col items-center shadow">
                ...
                <span className="text-2xl font-bold text-red-400">
                  {formatCurrency(statsTotalExpenses)}
                </span>
              </div>
              <div className="bg-blue-900/80 rounded-xl p-6 text-white flex flex-col items-center shadow">
                <span className="text-lg font-semibold mb-2">
                ...
                <span className="text-2xl font-bold text-yellow-300">
                  {formatCurrency(statsTotalSavings)}
                </span>
              </div>
              <div className="bg-blue-900/80 rounded-xl p-6 text-white flex flex-col items-center shadow">
                <span className="text-lg font-semibold mb-2">
                ...
                <span className="text-2xl font-bold text-purple-300">
                  {formatCurrency(statsTotalInvestments)}
                </span>
              </div>
              <div className="bg-blue-900/80 rounded-xl p-6 text-white flex flex-col items-center shadow">
                <span className="text-lg font-semibold mb-2">
                ...
                <span
                  className={`text-2xl font-bold ${statsNetCashFlow >= 0 ? "text-green-400" : "text-red-400"}`}
                >
                  {formatCurrency(statsNetCashFlow)}
                </span>
              </div>
            </div>
            {/* Cash Flow Graph */}
            <div className="bg-gray-900 rounded-xl p-8 shadow mb-8">
              <h3 className="text-xl font-bold text-white mb-4">
                Cash Flow (
                {filteredMonths[0]?.toLocaleString("default", {
                  month: "short",
                  year: "2-digit",
                })}{" "}
                -{" "}
                {filteredMonths[filteredMonths.length - 1]?.toLocaleString(
                  "default",
              ...
              <div className="w-full h-64 flex items-end gap-4">
                {statsCashFlowData.map((d, i) => {
                  const max = Math.max(
                    ...statsCashFlowData.map((x) =>
                      Math.max(x.income, x.expenses, x.savings, x.investments),
                    ),
                  );
                  ...
                  const investmentsHeight = max
                    ? (d.investments / max) * 180
                    : 0;
                  return (
                    <div key={i} className="flex flex-col items-center w-1/6">
                      <div className="flex flex-row items-end h-44 gap-1">
                        ...
                        ></div>
                        <div
                          className="bg-red-400 rounded-t w-3"
                          style={{ height: `${expensesHeight}px` }}
                          title={`Expenses: ${formatCurrency(d.expenses)}`}
                        ></div>
                        <div
                          className="bg-yellow-300 rounded-t w-3"
                          style={{ height: `${savingsHeight}px` }}
                          title={`Savings: ${formatCurrency(d.savings)}`}
                        ></div>
                        <div
                          className="bg-purple-300 rounded-t w-3"
                          style={{ height: `${investmentsHeight}px` }}
                          title={`Investments: ${formatCurrency(d.investments)}`}
                        ></div>
                      </div>
              ...
              <div className="flex justify-center gap-8 mt-4 flex-wrap">
                <div className="flex items-center gap-2">
                  <span className="w-4 h-2 bg-green-400 rounded inline-block"></span>{" "}
                  <span className="text-xs text-gray-200">Income</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-4 h-2 bg-red-400 rounded inline-block"></span>{" "}
                  <span className="text-xs text-gray-200">Expenses</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-4 h-2 bg-yellow-300 rounded inline-block"></span>{" "}
                  <span className="text-xs text-gray-200">Savings</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-4 h-2 bg-purple-300 rounded inline-block"></span>{" "}
                  <span className="text-xs text-gray-200">Investments</span>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-3xl mx-auto py-12">
            ...
            {accounts.length > 0 ? (
                    ...
                    <div key={acct.account_id} className="mb-4">
                      ...
                      <div className="text-gray-300">
                        ...
                        {acct.balances?.current != null
                          ? formatCurrency(acct.balances.current)
                          : "N/A"}
                      </div>
                    </div>
                  ))}
                ...
                {transactions.length > 0 && (
                  <div className="bg-gray-800 rounded-xl p-6 shadow mb-6">
                    ...
                    <table className="min-w-full text-left text-gray-300">
                      ...
                      <tbody>
                        {Object.entries(getBudgetSummary(transactions)).map(
                          ([cat, total]) => (
              ...
              <div className="text-gray-400">
                No accounts linked yet. Link a bank account to see your data
                here.
              </div>
            )}
          </div>
        ...
        {activeSection === "settings" && (
          <div className="max-w-3xl mx-auto py-12">
            ...
            <div className="bg-gray-800 rounded-xl p-6 shadow mb-6">
              ...
              <button className="px-4 py-2 bg-yellow-500 text-gray-900 rounded font-semibold">
                Change Password
              </button>
            </div>
          </div>
            ...
            <div className="bg-gray-800 rounded-xl p-6 shadow mb-6 flex flex-col items-center">
              ...
              {(() => {
                const data = [
                  {
                    label: "Needs",
                    value: incomeSplit.needs,
                    color: "#60a5fa",
                  },
                  {
                    label: "Wants",
                    value: incomeSplit.wants,
                    color: "#fbbf24",
                  },
                  {
                    label: "Savings",
                ...
                const pieSegments = data.map((d, i) => {
                  ...
                  const endAngle =
                    ((cumulative + d.value) / total) * 2 * Math.PI;
              ...
              <div className="flex flex-wrap gap-4 justify-center mt-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  ></span>{" "}
                  <span className="text-white">
                    Investments ({incomeSplit.investments}%)
                  </span>
                </div>
              </div>
              ...
              {incomeSplit.needs > 30 && (
                <div className="mt-8 bg-blue-900/80 border-l-4 border-blue-400 p-4 rounded text-white shadow">
                  ...
                  <div>
                    Your current <span className="font-semibold">Needs</span>{" "}
                    allocation is{" "}
                    <span className="font-semibold">{incomeSplit.needs}%</span>.
                    Consider reducing it to{" "}
                    <span className="font-semibold">30%</span> or below. This
                    will allow you to increase your{" "}
                    <span className="font-semibold">Savings</span> and{" "}
                    <span className="font-semibold">Investments</span>, which is
                    better for your long-term financial health.
                  </div>
                </div>
              )}
            </div>
                  ...
                  selectedReceipts.map((file, idx) => (
                    <li key={idx} className="flex items-center gap-4 mb-2">
                      ...
                      <button
                        ...
                        onClick={() => handleUploadReceipt(idx)}
                        disabled={ocrLoading}
                      >
                        Upload
                      </button>
                  ...
                  {uploadedReceipts.map((file, idx) => (
                    <li key={idx} className="text-white mb-2">
                      ...
                      <div className="text-xs text-green-300 whitespace-pre-line mt-1">
                        {ocrResults.find((r) => r.name === file.name)?.text ||
        ...
        {activeSection === "goals" && (
          <div className="max-w-3xl mx-auto py-12">
            ...
            <div className="bg-gray-800 rounded-xl p-6 shadow mb-6">
              ...
              {statusMessage && (
                <div className="text-green-400 mt-2">{statusMessage}</div>
              )}
            </div>
          </div>
        )}
        {activeSection === "savingsbucket" && (
              ...
              <div className="w-full flex flex-col gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="border rounded px-4 py-3 w-full text-gray-700"
                  />
                </div>
                <div>
                ...
                <label className="flex items-center gap-2 mt-2 mb-2 px-3 py-2 rounded bg-blue-100 text-blue-800 font-medium w-fit">
                  <input
                    type="checkbox"
                    checked={savingsAutoSend}
                    onChange={(e) => setSavingsAutoSend(e.target.checked)}
                    className="form-checkbox h-4 w-4 text-blue-500"
                  />
                  Auto Send Funds
                </label>
                <button
                  onClick={() => {
                    if (!savingsPurpose || !savingsAmount) return;
                    setSavingsBuckets([
                      ...savingsBuckets,
                      {
                        purpose: savingsPurpose,
                        amount: savingsAmount,
                        autoSend: savingsAutoSend,
                        enabled: true,
                      },
                    ]);
                    setSavingsPurpose("");
                    setSavingsAmount("");
                    setSavingsAutoSend(false);
                  }}
                  className="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg w-full"
                >
                  Add Bucket
                </button>
              </div>
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="text-gray-500 text-sm">
                            Auto Send:{" "}
                            {bucket.autoSend ? "Enabled" : "Disabled"}
                          </div>
                        </div>
                        <button
                          onClick={() => {
                            setSavingsBuckets(
                              savingsBuckets.map((b, i) =>
                                i === idx ? { ...b, autoSend: !b.autoSend } : b,
                              ),
                            );
                          }}
                          className={`px-4 py-2 rounded-lg font-semibold ${bucket.autoSend ? "bg-green-500 text-white" : "bg-gray-300 text-gray-700"}`}
                        >
                          {bucket.autoSend ? "Enabled" : "Disabled"}
                        </button>
                      </div>
                      <div className="flex gap-4 mt-4">
                        <button
                          ...
                          onClick={() => {
                            setModalMessage(
                              "Do you accept instant withdrawal with a fee?",
                            );
                            setShowModal(true);
                          }}
                        >
                        ...
                        <button
                          ...
                          onClick={() => {
                            setModalMessage(
                              "Do you accept delayed withdrawal with 2 days time?",
                            );
                            setShowModal(true);
                          }}
                        >
                          Delayed Withdrawal (2 days, no fee)
                        </button>
                      </div>
                    </div>
                  ))}
  ...
  const pieSegments = categories.map((cat, i) => {
    ...
    return (
      <g key={cat}>
        ...
        {value > 0 && (
          <text
            x={cx + radius * 0.7 * Math.sin((startAngle + endAngle) / 2)}
            y={cy - radius * 0.7 * Math.cos((startAngle + endAngle) / 2)}
            textAnchor="middle"
            dominantBaseline="middle"
            fontSize="12"
            fill="#fff"
...
function TimelineHorizon({ transactions }) {
  const timeline = useMemo(
    () => getLast30DaysTimeline(transactions),
    [transactions],
  );
